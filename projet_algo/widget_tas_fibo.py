from PyQt6.QtWidgets import QWidget, QScrollArea, QLabel,QVBoxLayout,QFrame,QPushButton,QFileDialog,QTableWidget,QTableWidgetItem
from PyQt6.QtGui import QPixmap,QMovie
from PyQt6.QtCore import Qt
import shutil
import os
from home_pages import home_pages
class learn_fibo_widget(QWidget):
    def __init__(self):
        super().__init__()
        ...
        label_definition_titre = QLabel("1.Explication Algorithme Tas de Fibonacci", content_widget)
        label_definition_titre.setStyleSheet("color:red;font-size:16px")
        label_definition_titre.setGeometry(10, 0, 290, 30)  #
        #
        #pour telecharge les cours 
        self.label_description_telcharge=QLabel("clic ici pour telcharge le cours de Tas de Fibonacci de Prof OQAIDI",content_widget)
        self.label_description_telcharge.setGeometry(272,30,350,17)
        ...
        self.nom_ficihier=QLabel("Séance_6_Algo-avancée.pdf",content_widget)
        ...
        label_definition = QLabel(
            "L'algorithme du tas de Fibonacci (Fibonacci Heap) est une structure de\ndonnées avancée utilisée pour optimiser des algorithmes de graphes\ncomme Dijkstra ou Prim. C'est un type de tas (heap) qui\npermet de maintenir une collection d'éléments, où l'on peut\nefficacement effectuer certaines opérations comme \nl'insertion et la diminution des clés.", content_widget
        )
        label_definition.setStyleSheet("color:black;font-size:12px;")
        label_definition.setGeometry(10, 170, 570, 110)  
        
        images_definition_dijikstre = QMovie("projet/ai.gif")
       ...
       #table
        ...
        self.table.setHorizontalHeaderLabels(["Opération", "Complexité amortie"])
        self.table.setItem(0, 0, QTableWidgetItem("Insertion"))
        self.table.setItem(1, 0, QTableWidgetItem("Trouver le minimum"))
        self.table.setItem(2, 0, QTableWidgetItem("Fusion (Union)"))
        self.table.setItem(3, 0, QTableWidgetItem("Extraire le minimum"))
        self.table.setItem(4, 0, QTableWidgetItem("Diminuer une clé"))
        self.table.setItem(5, 0, QTableWidgetItem("Suppression d'un nœud"))
        self.table.setItem(0, 1, QTableWidgetItem("O(1)"))
        self.table.setItem(1, 1, QTableWidgetItem("O(1)"))
        self.table.setItem(2, 1, QTableWidgetItem("O(1)"))
        self.table.setItem(3, 1, QTableWidgetItem("O(logn)"))
        self.table.setItem(4, 1, QTableWidgetItem("O(1)"))
        self.table.setItem(5, 1, QTableWidgetItem("O(logn)"))
        ...
        """)
        self.table.setGeometry(10,280,418,206)
